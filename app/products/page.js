...
export async function generateMetadata({ searchParams }) {
  ...
  let title = 'Tất cả sản phẩm | Lotus Spa';
  if (categorySlug) {
    const cats = await query('SELECT name FROM shop_categories WHERE slug = ?', [categorySlug]);
    if (cats.length > 0) {
      title = `${cats[0].name} | Lotus Spa`;
    }
  } else if (searchQ) {
    title = `Tìm kiếm: "${searchQ}" | Lotus Spa`;
  }
  
  return {
    title,
    description: 'Dịch vụ nha khoa chuẩn quốc tế: trồng răng Implant, niềng răng thẩm mỹ, răng sứ, tẩy trắng răng tại Lotus Spa.'
  };
}
